 ...
 * Raycasted as type 'foundry'; opens smelting panel on interact.
 * Builds procedurally; swaps to GLB from assets/models/stations/foundry.glb when available.
...
export class FoundryStation {
  constructor(id, x, z) {
    ...
    loadModel('/assets/models/stations/foundry.glb').then(model => {
      if (!model) return
      this.object.clear()
  ...
  _buildProcedural() {
    ...
    const fireMat = new THREE.MeshStandardMaterial({ color: 0xff6b1a, emissive: 0xff4500, emissiveIntensity: 1.4, roughness: 0.3 })
    ...
    this.object.add(body, mouth, fire, chimney, aBase, aTop, light)
  }
}
