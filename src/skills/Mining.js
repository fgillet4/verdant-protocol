...
const MINE_REACH   = 3.5    // covers diagonal navmesh cell (cellSize×√2 ≈ 2.83)
...
export class Mining {
  ...
  update(delta) {
    ...
    if (!this._started) {
      this._started = true
      this._setStatus(`You begin mining the ${node.tier?.name ?? 'ore'}.`)
    }
  ...
  _startTarget(nodeId) {
    ...
    const oreName  = node.tier?.name ?? 'ore'
    if (level < minLvl) {
      this._setStatus(`You need level ${minLvl} Mining to mine the ${oreName}.`)
      return
    }
    ...
    this._setStatus(`Walking to ${oreName}...`)
  }
